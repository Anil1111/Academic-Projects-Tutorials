...
module.exports = (done) => {
  db.serialize(function () {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS Strip(
      ...
      caption TEXT NOT NULL);`, err => {
      if (err) {
        throw err;
      }
    });
    db.run("INSERT INTO Strip (head, body, bubble_type, background, bubble_text, caption) VALUES ('angry', 'plus', 'statement', 'boat', 'test1', 'testcaption1')", err => {
      if (err) {
        throw err;
      }
    });
    db.run("INSERT INTO Strip (head, body, bubble_type, background, bubble_text, caption) VALUES ('angry', 'plus', 'statement', 'boat', 'test2', 'testcaption2')", err => {
      if (err) {
        throw err;
      }
    });
